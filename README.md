...
- 将服务端结果与智能体报告逐条交叉核对；未解释、未报告或未核验的偏离会阻止“完成/一致”，已解释偏离也不会自动改写目标。
- `get_review_status` 默认返回低成本摘要，需要时才读取逐项偏离、目标项、实际项、证据和解释。
...
实施核验中的“已解释偏离”只表示报告覆盖了差异，不表示架构目标已经改变。若代码需要成为新目标，仍必须重新提交目标提案并经过“接受草案 → 人工发布”。
...
| `get_review_status` | 查询精简审阅/核验摘要，按需读取逐项偏离详情 | 否 |
...
- `implementation-reconcile`：把实际代码与运行锁定的已发布正式目标对照，先提交实施后快照，再提交测试结果、完成度和全部偏离。

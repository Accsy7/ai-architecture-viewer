...
- 画布顶部用单行摘要提示草稿相对正式版的净变化；展开后在原右侧详情区按模块、关系/权限边界、验收条件分组查看字段、来源和依据，不会再用浮层遮挡画布。布局拖动不计入语义变化。
...
- 页面角落只显示一个目标语言按钮：中文界面显示 `English`，英文界面显示“中文”，并记住选择；只翻译查看器通用外壳，不翻译项目名称、图、模块、关系、文档、证据和用户原文。
- 桌面非全屏工作区固定为 880px 高，画布和右侧栏共同使用这一高度；旧项目只有 `meta.capabilityDomains` 时会无损兼容为区域分组，卡片归属仍由稳定 `group` 字段决定。
...
- v0.6.1 仅监听 `127.0.0.1`，变更 API 尚无身份验证、CSRF 防护或多用户授权。不要将其反向代理到局域网或互联网。
